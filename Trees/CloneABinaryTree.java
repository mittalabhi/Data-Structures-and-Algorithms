...
class Solution{
    public static Tree cloneTree(Tree tree, HashMap<Tree,Tree> hm){
        if(tree == null) return null;
        
        Tree cloneNode = new Tree(tree.data);
        hm.put(tree,cloneNode);
        cloneNode.left = cloneTree(tree.left,hm);
        cloneNode.right = cloneTree(tree.right,hm);
        return cloneNode;
        
    }
    public static void setRandom(Tree tree,Tree cloneTree,HashMap<Tree,Tree> hm){
        if(cloneTree == null) return;
        
        cloneTree.random = hm.get(tree.random);
        
        setRandom(tree.left,cloneTree.left,hm);
        setRandom(tree.right,cloneTree.right,hm);
    }
    public static Tree cloneTree(Tree tree){
       // add code here.
       HashMap<Tree,Tree> hm = new HashMap<>();
       
       Tree clonedTree = cloneTree(tree,hm);
       
       setRandom(tree,clonedTree,hm);
       return clonedTree;
     }
}